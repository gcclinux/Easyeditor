...
const boxClientId = defineSecret("BOX_CLIENT_ID");
const boxClientSecret = defineSecret("BOX_CLIENT_SECRET");
...
function buildBody(req: any, clientId: string, clientSecret: string): string {
...
export const boxOAuthProxy = onRequest(
  { secrets: [boxClientId, boxClientSecret], region: "europe-west1", invoker: "public" },
  async (req, res) => {
...
export const boxRevokeProxy = onRequest(
  { secrets: [boxClientId, boxClientSecret], region: "europe-west1", invoker: "public" },
  async (req, res) => {
    if (setCorsHeaders(req, res)) return;
